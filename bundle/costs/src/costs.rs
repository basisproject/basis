...
impl Costs {
    ...
    pub fn track(&mut self, prod: &str, val: f64) {
        ...
        let entry = self.raw.entry(prod.to_string()).or_insert(0.0);
        *entry += val;
    }
    ...
    pub fn raw(&self) -> &HashMap<String, f64> {
        &self.raw
    }

    #[allow(dead_code)]
    pub fn get(&self, product: &str) -> f64 {
        *self.raw.get(product).unwrap_or(&0.0)
    }
    ...
    pub fn take(&mut self, costs: &Costs) -> Costs {
        ...
        for (k, lval) in self.raw.iter_mut() {
            let mut rval = costs.raw().get(k).unwrap_or(&0.0) + 0.0;
...
impl Add for Costs {
    ...
    fn add(mut self, other: Self) -> Self {
        ...
        for k in other.raw().keys() {
            let entry = self.raw.entry(k.to_owned()).or_insert(0.0);
            *entry += other.raw().get(k).unwrap();
        }
        self
    }
}
...
impl Mul for Costs {
    ...
    fn mul(mut self, rhs: Self) -> Self {
        ...
        for (k, val) in self.raw.iter_mut() {
            *val *= rhs.raw().get(k).unwrap_or(&0.0);
        }
        self
    }
}
...
impl Mul<f64> for Costs {
    ...
    fn mul(mut self, rhs: f64) -> Self {
        self.labor *= rhs;
        for (_, val) in self.raw.iter_mut() {
            *val *= rhs;
        }
        self
    }
...
impl Div for Costs {
    ...
    fn div(mut self, rhs: Self) -> Self::Output {
        self.labor /= rhs.labor();
        for (k, v) in self.raw.iter_mut() {
            let div = rhs.raw().get(k).unwrap_or(&0.0);
        ...
        for (k, _) in rhs.raw().iter() {
            match self.raw.get(k) {
                None => {
                    self.raw.insert(k.clone(), 0.0);
                }
                _ => {}
            }
        }
